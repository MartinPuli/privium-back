...
public interface IAuthRepository extends JpaRepository<ResponseDto, Integer> {
    ...
    @Query(value = "CALL SetEmailConfirmationToken(:userId, :token, :proofMessage, :proofDocUrl)", nativeQuery = true)
    ResponseDto setEmailConfirmationToken(
            @Param("userId") Long userId,
            @Param("token") String token,
            @Param("proofMessage") String proofMessage,
            @Param("proofDocUrl") String proofDocUrl);
