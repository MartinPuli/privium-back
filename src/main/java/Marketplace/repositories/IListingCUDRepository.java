...
public interface IListingCUDRepository extends JpaRepository<ResponseDto, Integer> {
    ...
    @Query(value = "CALL ManageStatus(:action, :listingId)", nativeQuery = true)
    ResponseDto manageStatus(
            @Param("action") String action,
            @Param("listingId") Long listingId);

    @Query(value = "CALL SetAuxImages(:listingId, :images)", nativeQuery = true)
