...
public interface IListingRepository extends JpaRepository<Listing, Integer> {
        ...
        IListingDto addListing(
                        ...
                        @Param("condition") Integer condition, // 1=usado, 2=nuevo
        ...
        @Query(value = "EXEC GetAuxImages :listingId", nativeQuery = true)
        List<IListingImageDto> getAuxImages(
                        @Param("listingId") Long listingId) throws SQLException;

        @Query(value = "EXEC GetListingImages :listingId", nativeQuery = true)
        IListingImagesUrlsDto getListingImages(@Param("listingId") Long listingId) throws SQLException;

        @Query(value = "EXEC GetListingCategories :listingId", nativeQuery = true)
        List<IListingCategoryDto> getListingCategories(
                        @Param("listingId") Long listingId) throws SQLException;
}
